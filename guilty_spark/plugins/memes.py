...
def on_message(message):
    ...
    if '!bindmeme' in message.content:
        ...
        args = content.split('::')
        if len(args) != 3:
            yield from bot.send_message(message.channel, usage)
            return
        ...
        if len(trigger) < 3:
            yield from bot.send_message(
                message.channel, 'Trigger needs to be more then 3 '
                                 'characters')
            return

        dreams['memes'][meme_type][trigger] = meme
        cache_memes()
        yield from bot.send_message(message.channel, 'Meme bound')
        return

    memes = dreams['memes']
    if message.content in memes['is']:
        yield from bot.send_message(message.channel,
                                    memes['is'][message.content])
        return
